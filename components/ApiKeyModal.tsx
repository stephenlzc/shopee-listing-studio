...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="bg-gray-50 dark:bg-[#1a1a1f] border border-purple-300 dark:border-purple-500/30 rounded-2xl max-w-md w-full p-8 shadow-2xl shadow-purple-900/20 animate-in fade-in zoom-in duration-300">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">設定 API Key</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">API Key</label>
            <div className="relative">
                <input 
                    ...
                    placeholder="sk-..."
                    className={`w-full bg-gray-100 dark:bg-black/30 border rounded-lg px-4 py-3 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none transition-colors pr-10 ${
                      validationError ? 'border-red-500' : 'border-gray-300 dark:border-white/10 focus:border-purple-500'
                    }`}
                    required
                />
                {validationError && (
                  <p className="text-red-600 dark:text-red-400 text-xs mt-1">{validationError}</p>
                )}
                <button 
                    type="button"
                    onClick={() => setIsVisible(!isVisible)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-white"
                >
                    {isVisible ? (
