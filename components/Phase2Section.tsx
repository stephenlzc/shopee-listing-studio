  ...
  return (
    <div className="border border-white/5 rounded-xl overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between p-4 bg-[#1a1a1f] hover:bg-[#1e1e24] transition-colors text-left"
      >
        <span className="text-sm font-bold text-white">{title} <span className="text-gray-500 font-normal ml-2">({count})</span></span>
        <svg className={`w-4 h-4 text-gray-400 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      {open && <div className="p-4 bg-[#15151a]">{children}</div>}
    </div>
  );
};
...
const SeoTitleRow: React.FC<{ title: SeoTitle; onChange: (seq: number, value: string) => void }> = ({ title, onChange }) => (
  <div className="mb-3 last:mb-0">
    ...
    <textarea
      ...
      className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-purple-500 focus:outline-none resize-none h-16"
    />
    <p className="text-[11px] text-gray-500 mt-1">關鍵詞：{title.keywords}</p>
  </div>
);
  ...
  return (
    <div className="border-t border-white/10 pt-12 mt-12">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-2xl font-bold text-white flex-1">Phase 2: 素材預處理 + Listing 審閱</h2>
        <button
          onClick={onDownloadReport}
          className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm text-gray-300 hover:text-white transition-all flex items-center gap-2"
        >
      ...
      {/* Tab Navigation */}
      <div className="flex gap-1 mb-6 bg-[#1a1a1f] p-1 rounded-lg border border-white/10 w-fit">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${
              activeTab === tab.key ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white'
            }`}
          >
            {tab.label}
            {tab.badge && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                tab.badge === '已處理' ? 'bg-green-500/20 text-green-400' : 'bg-white/10 text-gray-300'
              }`}>
                {tab.badge}
              </span>
            )}
          ...
          <div>
            ...
            {processedImageBase64 && (
              <div className="mt-4 p-3 bg-green-500/10 border border-green-500/30 rounded-lg flex items-center gap-2">
                ...
                <span className="text-sm text-green-400 flex-1">模糊處理完成 — 已儲存處理後底圖</span>
              </div>
            )}

            {processedImageBase64 && !baseImageBase64 && (
              <div className="mt-4 p-4 bg-[#1a1a1f] border border-blue-500/20 rounded-xl">
            ...
            {baseImageBase64 && (
              <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg flex items-center gap-2">
                ...
                <span className="text-sm text-blue-400">產品底圖已生成 — 後續圖片將以此為參考</span>
              </div>
            )}
          </div>
        )}
          ...
          <>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-[#15151a] border border-white/10 rounded-xl p-4 text-sm text-white focus:border-purple-500 focus:outline-none resize-none h-32 leading-relaxed"
              />
            </div>
            ...
            <CollapsibleSection title="主圖 Prompt" count={listing.mainImages.length} defaultOpen>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {listing.mainImages.map((img) => (
                  <div key={img.id} className="bg-black/20 rounded-lg p-3 border border-white/5">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-white">{img.id} · {img.title}</span>
            ...
            {listing.complianceCheck.length > 0 && (
              <div>
                <h3 className="text-md font-bold text-white mb-3">合規檢查</h3>
                <div className="bg-[#15151a] border border-white/10 rounded-xl p-4">
                  <div className="space-y-2">
                    {listing.complianceCheck.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 text-sm">
                        <span className="text-red-400 line-through">「{item.originalWord}」</span>
                        <svg className="w-4 h-4 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                        </svg>
                        <span className="text-green-400">「{item.replacement}」</span>
                        <span className="text-xs text-gray-500 ml-auto">{item.riskReason}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {listing.skuBundles.map((sku, i) => (
                    <div key={i} className="bg-black/20 rounded-lg p-3 border border-white/5">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-bold text-white">{sku.emojiName}</span>
                        <span className="text-xs text-gray-500">{sku.sku}</span>
                      </div>
                      <p className="text-xs text-gray-400">{sku.contents}</p>
                      <p className="text-xs text-gray-500 mt-1">適合：{sku.targetBuyer} · {sku.guidance}</p>
                    </div>
                  ))}
                </div>
              </CollapsibleSection>
